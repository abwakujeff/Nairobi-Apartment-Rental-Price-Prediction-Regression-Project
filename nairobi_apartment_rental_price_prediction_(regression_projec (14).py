...
Since Price is a continuous numerical target, this is a regression problem. We choose:
* Linear Regression (baseline linear model).
* Random Forest Regressor (an ensemble of decision trees).
* XGBoost Regressor (gradient boosting trees, often effective on tabular data).
Tree ensemble models like XGBoost are particularly powerful for tabular regression problems
arxiv.org  
We will train each and compare.
"""
...
"""#Model Evaluation
We evaluate each model on the test set using R², RMSE, and MAE.
...
RMSE (root mean squared error) is the standard deviation of residuals (prediction errors)
geeksforgeeks.org  
.
MAE (mean absolute error) is the average absolute difference between predictions and true values
geeksforgeeks.org
.
"""
